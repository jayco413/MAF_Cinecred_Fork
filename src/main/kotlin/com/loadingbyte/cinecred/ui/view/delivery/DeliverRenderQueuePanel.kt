...
class DeliverRenderQueuePanel(deliveryCtrl: DeliveryCtrlComms) : JScrollPane(), DeliveryViewComms {
        ...
        val jobTable = JTable(jobTableModel).apply {
            ...
            columnModel.apply {
                ...
                getColumn(5).apply {
                    ...
                    cellEditor = DestinationLinkCell { rowIdx -> tryOpen(jobTableModel.rows[rowIdx].info.job.prefix) }
                }
    ...
    private class JobTableModel : AbstractTableModel() {
        ...
        // The cancel button column is editable so the cancel buttons are clickable, and the destination column is
        // editable once the render succeeded so that clicking it opens the rendered file.
        override fun isCellEditable(rowIndex: Int, colIndex: Int) =
            colIndex == 7 || colIndex == 5 && rows[rowIndex].status is RenderJobStatus.Done
    ...
    /** Renders the destination of a finished job as a link that opens the rendered file when clicked. */
    private class DestinationLinkCell(private val callback: ((Int) -> Unit)? = null) :
        ...
        override fun getTableCellRendererComponent(
            ...
            val done = (table.model as JobTableModel).rows[rowIdx].status is RenderJobStatus.Done
            return if (!done)
        ...
        override fun getTableCellEditorComponent(
            ...
            callback?.invoke(rowIdx)
            // The cell is only used to trigger the click, so stop editing again right away.

...
class DeferredImage(var width: Double = 0.0, var height: Y = 0.0.toY()) {
    ...
    fun materialize(
        ...
        layers: List<Layer>,
        flashing: Flashing = Flashing.STATIC
    ) {
    ...
    fun materialize(tracker: PDFTracker, page: PDPage, cs: PDPageContentStream, layers: List<Layer>) {
        ...
        materialize(backend, null, layers, Flashing.STATIC)
    }
    ...
    /** Whether any of the given layers contains text or shapes painted with a [Coat.Flashing] coat. */
    fun hasFlashingText(layers: List<Layer>): Boolean =
        collectFlashingStateIndices(this, layers, frameIdx = 0, out = null)

    /**
     * Identifies which flashing color each flashing coat shows at [frameIdx]. Two frames that yield the same key are
     * pixel-identical as far as flashing text is concerned, which lets callers reuse cached renders.
     */
    fun flashingStateKey(layers: List<Layer>, frameIdx: Int): FlashingStateKey? {
        val out = mutableListOf<Int>()
        return if (collectFlashingStateIndices(this, layers, frameIdx, out)) FlashingStateKey(out) else null
    }

    private fun materialize(
        backend: MaterializationBackend, culling: Rectangle2D?, layers: List<Layer>, flashing: Flashing
    ) {
        for (layer in layers)
            Instruction.DrawDeferredImageLayer(0.0, 0.0.toY(), 1.0, 1.0, this, layer)
                .materialize(backend, 0.0, 0.0, 1.0, 1.0, culling, flashing)
    }
    ...
    companion object {
        ...
        private fun Coat.toShader() = when (this) {
            ...
            // Flashing coats are resolved to Plain ones before they reach a backend; this is only a safety net.
        ...
        /**
         * Walks the instruction tree and reports whether any flashing coat was found, appending the index of the color
         * each one shows at [frameIdx] to [out] when it is given.
         */
        private fun collectFlashingStateIndices(
            image: DeferredImage, layers: List<Layer>, frameIdx: Int, out: MutableList<Int>?
        ): Boolean {
            var found = false
            for (layer in layers)
                for (insn in image.instructions.getOrDefault(layer, emptyList())) {
                    val coat = when (insn) {
                        is Instruction.DrawDeferredImageLayer -> {
                            if (collectFlashingStateIndices(insn.image, listOf(insn.layer), frameIdx, out)) {
                                found = true
                                if (out == null) return true
                            }
                            continue
                        }
                        is Instruction.DrawShape -> insn.coat
                        is Instruction.DrawText -> insn.coat
                        else -> continue
                    }
                    if (coat is Coat.Flashing) {
                        found = true
                        if (out == null) return true
                        out.add(Flashing.colorIdx(coat, frameIdx))
                    }
                }
            return found
        }
    ...
    sealed interface Coat {
        ...
        /**
         * Text that cycles through [colors], switching every [intervalFrames] frames. Only ever materialized as a
         * [Plain] coat; see [Flashing] for how the shown color is picked.
         */
    ...
    /**
     * Configures how [Coat.Flashing] coats are materialized. Static outputs (SVG, PDF, previews) use [STATIC], which
     * always shows the first color; video delivery advances the color with the frame index.
     */
    class Flashing(
        private val frameIdx: Int = 0,
        private val animate: Boolean = false,
        private val mode: Mode = Mode.ALL
    ) {

        /** Whether the given coat should be drawn at all, used to render flashing text into a separate pass. */
        fun shouldDraw(coat: Coat): Boolean = when (mode) {
            Mode.ALL -> true
            Mode.EXCLUDE -> coat !is Coat.Flashing
            Mode.ONLY -> coat is Coat.Flashing
        }

        /** Collapses a flashing coat down to the plain color it shows at this point in time. */
        fun resolve(coat: Coat): Coat = when (coat) {
            is Coat.Flashing -> Coat.Plain(coat.colors[if (animate) colorIdx(coat, frameIdx) else 0])
            else -> coat
        }
        ...
        companion object {
            val STATIC = Flashing()
            internal fun colorIdx(coat: Coat.Flashing, frameIdx: Int) =
                (frameIdx / coat.intervalFrames).mod(coat.colors.size)
        }

    }


    /** Identifies the combination of colors that all flashing coats of an image show at a given frame. */
    ...
    private sealed interface Instruction {
        ...
        class DrawDeferredImageLayer(
            ...
            override fun materialize(
                ...
                for (insn in image.instructions.getOrDefault(layer, emptyList()))
                    insn.materialize(backend, x, y, universeScaling, elasticScaling, culling, flashing)
            }
        }
            ...
            override fun materialize(
                ...
                if (culling == null ||
                    ...
                    backend.materializeShape(
                        shape.transformedBy(tx), flashing.resolve(coat).transform(tx), fill, dash = false, blurRadius
                    )
                }
            }
        }
        ...
        class DrawText(
            ...
            override fun materialize(
                ...
                if (culling == null ||
                    ...
                    backend.materializeText(x, yBaseline, universeScaling, text, flashing.resolve(coat))
            }
        }
    ...
    private class SVGBackend(
        ...
        private fun applyCoat(coatedElement: Element, coat: Coat, fill: Boolean) {
            when (coat) {
                is Coat.Flashing -> applyCoat(coatedElement, Flashing.STATIC.resolve(coat), fill)
                is Coat.Plain -> {
    ...
    private class PDFBackend(
        ...
        private fun setCoat(coat: Coat, fill: Boolean, bbox: Rectangle2D) {
            when (coat) {
                is Coat.Flashing -> setCoat(Flashing.STATIC.resolve(coat), fill, bbox)
                is Coat.Plain -> {

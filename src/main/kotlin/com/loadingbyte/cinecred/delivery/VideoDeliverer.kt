...
class VideoDeliverer(
    ...
    /** When given, rendered chunks are cached on disk so that re-renders of the same project reuse them. */
    diskCache: RenderDiskCache.Session? = null,
    ...
    init {
        ...
        backend = DeferredVideo.BitmapBackend(
            ...
            // Deliveries are the only place where flashing text actually animates.
    ...
    fun deliverFrame(): Bitmap? {
        ...
        // Frames backed by a cached render come back as shared views. When we do not modify the frame, hand the
        // view straight to the caller (who copies it into the writer) instead of copying it here as well.
        val materialized = backend.materializeFrameForExport(frameIdx) ?: return null
        val colorBitmap = if (materialized.owned || matte) ownFrame(materialized) else materialized.bitmap
        return when {
            !matte -> colorBitmap
    ...
    /** Copies a shared frame view into a bitmap this class owns, so that callers may close or modify it. */
    private fun ownFrame(materialized: DeferredVideo.BitmapBackend.ExportMaterializedFrame): Bitmap {
        if (materialized.owned)
            return materialized.bitmap
        return profiling.measure("deliverer.copySharedFrame") {
            val owned = Bitmap.allocate(backendSpec)
            owned.blit(materialized.bitmap)
...
/**
 * Builds the string that identifies a render for the disk cache. Any change to it invalidates previously cached
 * chunks, so it must capture everything that influences the rendered pixels.
 */
internal fun renderKey(
    job: String,
    format: String,
    configIdx: Int,
    ...
    append("configIndex=").append(configIdx).append('\n')
    append("resolutionSlider=").append(sliders.resolution).append('\n')
    append("slate=").append(sliders.slate).append('\n')
    append("videoResolution=").append(video.resolution).append('\n')
    append("videoFps=").append(video.fps).append('\n')
    append("videoNumFrames=").append(video.numFrames).append('\n')
    append(spec).append('\n')
